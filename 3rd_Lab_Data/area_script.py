...
def main():
    ...
    area = getArea(args[0], args[1])
    print "area is %f mm^2" % area
    

def getArea(mcpatoutputFile, statsFile):
    area, areaL2 = readMcPAT(mcpatoutputFile)
    runtime = getTimefromStats(statsFile)
    print "core area: %f mm^2, L2 area: %f mm^2" % (area, areaL2)
    return area+areaL2
...
def readMcPAT(mcpatoutputFile):
    ...
    area = p.getValue(['Core:', 'Area'])
    areaL2 = p.getValue(['L2:', 'Area'])
    area = re.sub(' mm\\^2','', area) 
    areaL2 = re.sub(' mm\\^2','', areaL2) 

    return (float(area), float(areaL2))
