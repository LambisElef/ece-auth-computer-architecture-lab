...
def getEnergy(mcpatoutputFile, statsFile):
    peak, peakL2 = readMcPAT(mcpatoutputFile)
    print "peak: %f W, peakL2: %f W" % (peak, peakL2)
    return 10
...
def readMcPAT(mcpatoutputFile):
    ...
    peakDynamicL2 = p.getValue(['L2:', 'Peak Dynamic'])

    peak = re.sub(' W','', peak) 
    leakageSL2 = re.sub(' W','', leakageSL2) 
    leakageGL2 = re.sub(' W','', leakageGL2) 
    peakDynamicL2 = re.sub(' W','', peakDynamicL2) 

    return (float(peak), float(peakDynamicL2)+float(leakageSL2)+float(leakageGL2))
