...
def getEnergy(mcpatoutputFile, statsFile):
    leakageS, leakageG, dynamic, leakageSL2, leakageGL2, dynamicL2 = readMcPAT(mcpatoutputFile)
    runtime = getTimefromStats(statsFile)
    energy = (leakageS + leakageG + dynamic + leakageSL2 + leakageGL2 + dynamicL2)*runtime
    print "leakage: %f W, dynamic: %f W and runtime: %f sec" % (leakageS+leakageG+leakageSL2+leakageGL2, dynamic+dynamicL2, runtime)
    return energy*1000
...
def readMcPAT(mcpatoutputFile):
    ...
    leakageS = p.getValue(['Core:', 'Subthreshold Leakage'])
    leakageG = p.getValue(['Core:', 'Gate Leakage'])
    dynamic = p.getValue(['Core:', 'Runtime Dynamic'])
    leakageS = re.sub(' W','', leakageS) 
    leakageG = re.sub(' W','', leakageG) 
    dynamic = re.sub(' W','', dynamic) 

    leakageSL2 = p.getValue(['L2:', 'Subthreshold Leakage'])
    leakageGL2 = p.getValue(['L2:', 'Gate Leakage'])
    dynamicL2 = p.getValue(['L2:', 'Runtime Dynamic'])
    leakageSL2 = re.sub(' W','', leakageSL2) 
    leakageGL2 = re.sub(' W','', leakageGL2) 
    dynamicL2 = re.sub(' W','', dynamicL2) 

    return (float(leakageS), float(leakageG), float(dynamic), float(leakageSL2), float(leakageGL2), float(dynamicL2))
